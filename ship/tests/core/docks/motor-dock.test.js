describe('MotorDock', () => {
  ...
  let ship = new MotorShip('Potemkin', '123', position);
  let dock;
  beforeEach(() => {
    dock = new MotorDock(position, []);
  });
  ...
  it ('unsuccess moor if unsupported ship and so can\'t repair, paint, change if ship is not in Dock)', () => {
    assert.throws(() => { dock.moor(new SailingShip('Other', '1', position)) }, 'Ship is not supported for this Dock');
  });
